...
//    }
    static boolean isClone(String location0, String location1) {
        char col0 = (char) ('g' - (location0.charAt(0) - 'a'));
        char row0 = (char) ('7' - (location0.charAt(1) - '1'));
        char col1 = (char) ('g' - (location1.charAt(0) - 'a'));
        char row1 = (char) ('7' - (location1.charAt(1) - '1'));

        if (col0 == col1 && row0 == row1) {
            return false;
        }
        if (Math.abs(col1 - col0) > 1 || Math.abs(row1 - row0) > 1) {
            return false;
        }
        return true;
    }
...
//    }
    static boolean isJump(String location0, String location1) {
        ...
        if (Math.abs(col1 - col0) > 2 || Math.abs(row1 - row0) > 2) {
            return false;
        }
        return true;
    }
